                                    ...
                                    cases.data.map((c) => (
                                        <tr key={c.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-900/40 transition">
                                            ...
                                            <td className="py-3 px-4 font-sans text-slate-700 dark:text-slate-300">
                                                ...
                                                <div className="text-[10px] text-slate-400 dark:text-slate-500">{c.ddo_designation}</div>
                                            </td>
                                            ...
                                            <td className="py-3 px-4 font-sans">
                                                {c.is_closed ? (
                                                    <span className="px-2 py-0.5 text-[10px] font-bold bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20 rounded-full inline-flex items-center gap-1">
                                                        <CheckCircle2 className="w-3 h-3" />
                                                        <span>Closed on {c.closed_at}</span>
                                                    </span>
                                                ) : (
                                                    <span className="px-2 py-0.5 text-[10px] font-bold bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 border border-rose-200 dark:border-rose-500/20 rounded-full inline-flex items-center gap-1">
                                                        <ShieldAlert className="w-3 h-3" />
                                                        <span>Recovery Outstanding</span>
                                                    </span>
                                                )}
                                                {c.minus_balance_remarks && (
                                                    <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-1 truncate max-w-xs" title={c.minus_balance_remarks}>
                                                        {c.minus_balance_remarks}
                                                    </div>
                                                )}
                                            </td>
                                            ...
                                            <td className="py-3 px-4 text-center font-sans">
                                                <a
                                                    ...
                                                    className="px-2.5 py-1 bg-rose-50 hover:bg-rose-100 dark:bg-rose-500/10 dark:hover:bg-rose-500/20 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-500/30 rounded-lg text-[11px] font-medium transition inline-flex items-center gap-1"
                                                >
                        ...
                        <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex justify-end gap-1">
                            {cases.links.map((l, i) => (
                                <Link
                                    ...
                                    className={`px-3 py-1 text-xs rounded-lg transition ${l.active ? 'bg-rose-600 text-white shadow-2xs' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'} ${!l.url && 'opacity-40 cursor-not-allowed'}`}
                                />
                            ))}
                        </div>
                    )}
