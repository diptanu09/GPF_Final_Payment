                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 font-mono">
                                {signatures.data?.length > 0 ? (
                                    signatures.data.map((s) => (
                                        <tr key={s.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-900/40 transition">
                                            <td className="py-3 px-4 font-sans">
                                                <div className="font-bold text-slate-900 dark:text-slate-100 font-mono text-sm">{s.registration_no}</div>
                                                <div className="text-[11px] text-indigo-600 dark:text-indigo-400 font-mono font-medium">{s.gpf_account}</div>
                                            </td>
                                            ...
                                            <td className="py-3 px-4 font-mono text-[10px] text-indigo-600 dark:text-indigo-300">
                                                <div className="bg-slate-50 dark:bg-slate-950 p-1.5 rounded border border-slate-200 dark:border-slate-800 truncate max-w-xs shadow-2xs" title={s.signature_hash}>
                                                    {s.signature_hash}
                                                </div>
                                            </td>

                                            <td className="py-3 px-4 text-right text-slate-700 dark:text-slate-300 font-sans">
                                                {s.signed_at}
                                            </td>
                                        </tr>
                                    ))
                        ...
                        <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex justify-end gap-1">
                            {signatures.links.map((l, i) => (
                                <Link
                                    ...
                                    className={`px-3 py-1 text-xs rounded-lg transition ${l.active ? 'bg-purple-600 text-white shadow-2xs' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'} ${!l.url && 'opacity-40 cursor-not-allowed'}`}
                                />
                            ))}
                        </div>
                    )}
