@php
    ...
    $memoNo = "No. {$sectionName} / FP / Corrigendum / {$pensionType} / {$openingFinYear} / {$case->registration_no} /";

    $matter = $corrigendum_matter ?? ("Please read the name of Treasury / DDO as [Corrected Name] instead of [Old Name] wherever appeared in the GPF Final Payment Authority for Rs. " . number_format($finalAmount, 2) . " issued in favour of {$case->name_title} {$case->subscriber_name_cache}, {$case->designation_title} {$case->designation}, GPF A/c No. {$case->formatted_gpf_account} from this office vide Memo No. {$sectionName} / FP / {$pensionType} / {$openingFinYear} / {$case->registration_no} / dated " . ($authority?->authority_date ? $authority->authority_date->format('d/m/Y') : date('d/m/Y')) . ".\n\nAll other terms and conditions will remain unchanged.");

    $copyTo = $copy_to ?? ("1. The Treasury Officer, {$case->treasury_name} for information and necessary action.\n2. {$case->ddo_designation} ({$case->ddo_code}) for making necessary payment.\n3. {$case->name_title} {$case->subscriber_name_cache}, {$case->personal_address}.");
@endphp
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Corrigendum - {{ $case->registration_no }}</title>
    <style>
        @page {
            size: A4 portrait;
        ...
        .doc-title { text-align: center; font-weight: bold; text-decoration: underline; margin-bottom: 20px; font-size: 15px; letter-spacing: 1px; }
