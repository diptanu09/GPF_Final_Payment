                                       ...
                                       <tbody>
                                           <?php
                                            $inwardQuery = "SELECT a.REGD_NO, 'T/'||e.SERIES_DESCR||'/'||a.ACCOUNT_NO ACCOUNT_NO, b.SUBSCRIBER_NAME,
                                                            d.PENSION_LONG_DESCR, d.PENSION_SHORT_DESCR, b.CASE_STATUS FROM GPF_INWARD a INNER JOIN GPF_CASE_STATUS b  
                                                            ...
                                                            ON a.SERIES_ID=e.SERIES_ID WHERE b.CASE_STATUS NOT IN (7,8,10,11,16,17,18,19) AND a.MARK_TO='" . $_SESSION['GPF_USERNAME'] . "'
                                                            GROUP BY a.REGD_NO, e.SERIES_DESCR, a.ACCOUNT_NO, b.SUBSCRIBER_NAME, d.PENSION_LONG_DESCR, 
                                                            d.PENSION_SHORT_DESCR, b.CASE_STATUS ";
                                            $fetchinward = sqlFetchData($connection, $inwardQuery);
                                            foreach ($fetchinward as $inwardList) {
                                                $appQuery = "SELECT * FROM GPF_APPLICATION WHERE REGD_NO='" . $inwardList['REGD_NO'] . "'";
                                                       ...
                                                       <?php
                                                        if ($inwardList['CASE_STATUS'] >= 12) {
                                                            if ($inwardList['CASE_STATUS'] > 14) {
                                                                ...
                                                                if ($count > 0) {
                                                        ?>
                                                                   <a href="view_application_share_holders.php?regd_no=<?php echo $inwardList['REGD_NO']; ?>">
                                                                       <button class="btn btn-info">
                                                                           SHARE HOLDERS</button>
                                                                   </a>
                                                               <?php
                                                                }
                                                                ?> <a href="application.php?regd_no=<?php echo $inwardList['REGD_NO']; ?>">
                                                                   <button class="btn btn-<?php echo $btnbg; ?>">
                                                                       <?php
                                                                        echo $btnText;
                                                                        ?></button>
                                                               </a>
                                                           ...
                                                           <?php
                                                            if ($inwardList['CASE_STATUS'] > 5) {
                                                                if ($inwardList['CASE_STATUS'] == 6) {
                                                                    ...
                                                                    if ($inwardList['CASE_STATUS'] == 10) {
                                                            ?>
                                                                       <b class='badge badge-danger'>Minus balance. </b>
                                                                   <?php
                                                                    } else {
                                                                    ?>
                                                                       <b class='badge badge-primary'>Objectioned. </b>
                                                                   <?php
                                                                    }
                                                                    ?>
                                                                   <a href="view_application_share_holders.php?regd_no=<?php echo $inwardList['REGD_NO']; ?>">
