...
if (isset($_REQUEST['regd_no'])) {
    ...
    $infoSQL = "SELECT 'T/'||D.SERIES_DESCR||'/'||A.ACCOUNT_NO GPF_ACCOUNT_NO, C.TITLE, A.SUBSCRIBER_NAME, C.SPOUSE_NAME, K.SECTION_DESCR,
    A.APPROVED_DATE, C.DESG_TITLE, C.RELATION SPOUSE_RELATION, E.FIN_YEAR OPENING_FIN_YEAR, F.PENSION_SHORT_DESCR, 
    G.DDO_DESG, G.DDO_CODE, I.TRES_CODE TREASURY_CODE, I.TRES_NAME TREASURY, H.TRES_CODE SUB_TREASURY_CODE, C.LTA_TO_WHOM,
                                        ...
                                        <div class="col-lg-8" id="printArea">
                                            ...
                                            <table style="width: 100%;">
                                                <tr>
                                                    <td style="width: 60%; text-align:left;">
                                                        No. <?php echo $sectionName; ?> / LTA / <?php echo $pensionType; ?> / <?php echo $openingFinYear; ?> / <?php echo $registrationNo; ?> /
                                                    </td>
                                                    <td style="width: 40%; text-align:right;">Date : <?php echo date("d / m / Y", strtotime($approvedDate)); ?></td>
                                                </tr>
                                            </table><br>
                                            <div style="text-align: center; font-weight:bolder; text-decoration:underline;">Authorization letter</div>
                                            <div style="text-align:justify; line-height:1.8em; padding:10px;">
                                                <!-- <div class="rotate">Below Rupees <?php //echo trim(convertToWords($finalPaymentAmount + 1)) 
                                                                                        ?> only</div> -->
                                                <!-- <h1 id="watermark" class="watermark">LTA OF GPF FINAL PAYMENT AUTHORITY</h1> -->
                                                ...
                                                <table width="100%" style="text-align:center;" border="1" cellspacing=0 cellpadding="10px" class="table table-bordered">
                                                    <tr>
                                                        <th width="30%" style="font-weight: bolder;">O.B. as at the begining of the year <br />
                                                            ( Rs )</th>
                                                        <th width="30%" style="font-weight: bolder;">Subscription / Refund during the year <br />( Rs )</th>
                                                        <th width="10%" style="font-weight: bolder;">Withdrawal / Advance<br />( Rs )</th>
                                                        <th width="10%" style="font-weight: bolder;">Interest<br />( Rs )</th>
                                                        <th width="20%" style="font-weight: bolder;">Closing balance<br />( Rs )</th>
                                                    </tr>
                                                    <tr>
                                                ...
                                                5. Payment is subject to adjustment of any payment made by the DDO between the date of forwarding the application to the date of actual payment.<br />
                                                6. The whole amount may be paid to <?php
                                                                                    echo $ltaWhom . ' of ' . $subscriberInitial . " " . $subscriberName;
                                                                                    ?>.

                                                <br />
                                                7. The amount is payable on or after <span style="font-weight:bold;"><?php echo date("d-M-Y", strtotime($interestAllowed)); ?></span> only and authorisation is valid for six months from the date of issue.<br /><br />
                                                <div style="text-align: right;">Authorized Signatory</div> <br>
                                                <table>
                                                    <tr>
                                                        <td width="80%" valign="top">
                                                            Copy forwarded for information and necessary action to :-<br />
                                                            1. Treasury Officer - <?php echo $treasury ?><?php echo $subTresuryCode == $tresuryCode ? "($tresuryCode)" : ", payable to " . $subTresury . " ($subTresuryCode)" ?>.<br>
                                                            2. <?php echo $ddo; ?> (<?php echo $ddoCode; ?>).<br>
                                                            3. <?php echo $ltaWhom . ' of ' . $subscriberInitial . " " . $subscriberName; ?>, <?php echo $desgTitle . " " . $designation; ?>, <?php echo $personalAddress; ?>.
    ...
    const download = () => {
        ...
        jQuery.ajax({
            ...
            success: function(returnValue) {
                ...
                if (StatusCode === 200) {
                    // const urlLink = `digital_signature.php?regd_no=${registrationNo}&&type=lta_authority`;
