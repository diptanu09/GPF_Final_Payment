...
if (isset($_REQUEST['regd_no'])) {
    ...
    $applicationQuery = "SELECT 'T/'||D.SERIES_DESCR||'/'||A.ACCOUNT_NO GPF_ACCOUNT_NO, A.SUBSCRIBER_NAME, H.SECTION_DESCR, 
                        ...
                        B.DESIGNATION, B.TITLE, Y.TRES_NAME TREASURY, X.TRES_NAME SUB_TREASURY FROM GPF_CASE_STATUS A INNER JOIN GPF_APPLICATION B ON A.REGD_NO=B.REGD_NO
                        LEFT JOIN VLCS.STATE_TREASURY X ON B.TREASURY_CODE=X.TRES_CODE LEFT JOIN VLCS.STATE_TREASURY Y ON X.CNTR_TRES=Y.TRES_CODE
    ...
    foreach ($fetchApplicationData as $fetchApplicationList) {
        ...
        $fTreasury = $fetchApplicationList['TREASURY'];
        $fSubTreasury = $fetchApplicationList['SUB_TREASURY'];
    }
    ...
    Subject : Revalidation of GPF Final Payment Authority No. dated
    
    Sir,
                   In inviting a reference to the letter No.............. dated, ................ received from the $fTreasury, on the above mentioned subject, I am to furnish herewith the GPF Final Payment authority as was issued in favour of $fNameTitle $fsubscriberName holder of GPF A/c No. $fAccountNo amounting to ₹ $fFinalPayment /- only vide No .............................. dated, ..................... after necessary revalidation with a request to arrange for payment through the $fTreasury at an early date.
    ...
    $fCopyTo = "1. $fddoName.
               ...
               2. The treasury officer, $fTreasury for information and necessary action. He is requested to arrange for payment through the $fSubTreasury.

              3. $fNameTitle $fsubscriberName,  $fDesignationTitle  $fDesignation, $fpersonalAddress";
?>
                                            ...
                                            <div class="col-lg-8" id="printArea">
                                                ...
                                                <div style="text-align:justify; line-height:1.8em; padding:10px;">
                                                    <table style="width:100%;">
                                                        <tr>
                                                            <td style="text-align: left; width:48%;">No. <?php echo $memoNo; ?></td>
                                                            <td style="text-align: right; width:48%;">Date : <?php echo date("d / m / Y");
                                                                                                                ?></td>
                                                        </tr>
                                                    </table><br>
